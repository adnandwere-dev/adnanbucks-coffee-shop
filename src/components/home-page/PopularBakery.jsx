      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-16 lg:gap-24">
        ...
        <div className="md:w-1/2 space-y-8">
          ...
          <div className="space-y-0 pt-2">
            {PRODUCTS.filter((item) => item.category === "bakery").map((item) => (
              <div key={item.name} className="flex justify-between items-center border-b border-outline-variant/30 py-5">
                <div>
                  <h4 className="font-body text-lg text-on-surface font-bold">{item.name}</h4>
                  <p className="font-body text-sm text-on-surface-variant mt-1">{item.desc}</p>
                </div>
                <span className="font-body text-lg text-primary font-bold ml-4">{item.price}</span>
              </div>
            ))}
          </div>
